...
def make_loss(cfg, num_classes):    # modified by gu
    ...
    if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
        ...
        track_triplet = TripletLoss(cfg.SOLVER.TRACK_MARGIN)  # track triplet loss
    ...
    if sampler == 'softmax':
        def loss_func(score, feat, target, tid):
            return F.cross_entropy(score, target)
    elif cfg.DATALOADER.SAMPLER == 'triplet':
        def loss_func(score, feat, target, tid):
            return triplet(feat, target)[0] + cfg.SOLVER.TRACK_WEIGHT * track_triplet(feat, tid)[0]
    elif cfg.DATALOADER.SAMPLER == 'softmax_triplet':
        def loss_func(score, feat, target, tid):
            if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
                if cfg.MODEL.IF_LABELSMOOTH == 'on':
                    return xent(score, target) + triplet(feat, target)[0] + cfg.SOLVER.TRACK_WEIGHT * track_triplet(feat, tid)[0] # new add by luo, open label smooth
                else:
                    return F.cross_entropy(score, target) + triplet(feat, target)[0] + cfg.SOLVER.TRACK_WEIGHT * track_triplet(feat, tid)[0]   # new add by luo, no label smooth
                ...
                if cfg.MODEL.IF_LABELSMOOTH == 'on':
                    return xent(score, target) + triplet(feat, target)[0] + cfg.SOLVER.TRACK_WEIGHT * track_triplet(feat, tid)[0] + cluster(feat, target)[0]  # new add by luo, open label smooth
                else:
                    return F.cross_entropy(score, target) + triplet(feat, target)[0] + cfg.SOLVER.TRACK_WEIGHT * track_triplet(feat, tid)[0] + cluster(feat, target)[0]    # new add by luo, no label smooth
            else:
                print('expected METRIC_LOSS_TYPE should be triplet, cluster, triplet_cluster，'
                      'but got {}'.format(cfg.MODEL.METRIC_LOSS_TYPE))
    else:
...
def make_loss_with_center(cfg, num_classes):    # modified by gu
    ...
    def loss_func(score, feat, target, tid):
        if cfg.MODEL.METRIC_LOSS_TYPE == 'center':
            ...
            if cfg.MODEL.IF_LABELSMOOTH == 'on':
                return xent(score, target) + \
                        triplet(feat, target)[0] + cfg.SOLVER.TRACK_WEIGHT * track_triplet(feat, tid)[0] +\
                        cfg.SOLVER.CENTER_LOSS_WEIGHT * center_criterion(feat, target)  # new add by luo, open label smooth
            else:
                return F.cross_entropy(score, target) + \
                        triplet(feat, target)[0] + cfg.SOLVER.TRACK_WEIGHT * track_triplet(feat, tid)[0] +\
